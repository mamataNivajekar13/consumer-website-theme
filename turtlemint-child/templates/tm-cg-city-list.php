...
/**
* Template Name: Cashless Garages Listing
 */
...
<html <?php language_attributes(); ?>>
    <head>
        ...
        <?php
            ...
            $city_slug = (get_query_var( 'car-cg-city' ));
            ...
            $garageslist = getCashlessGaragesDataByCity($city_name, $insurer_code );
            ...
            $piece["itemListElement"] = [
                ...
                [
                    ...
                    "item" => $term_page_link."cashless-garages/"
                ],
                [
                    "@type" => "ListItem",
                    "position" => 5,
                    "name" => ucwords($insurer_display_name." Cashless Garages List".strtolower(" in ").ucwords($city_name)),
                    "item" => $currentURL
                ]
            ];
        ...
        <?php
            ...
            if($insurer_display_name){
                $keywords = $insurer_display_name.' cashless garages in '.$city_name.', '.$insurer_display_name.' car insurance cashless garages list '.$city_name.', '.$insurer_display_name.' cashless garages list in '.$city_name.', '.$insurer_display_name.' car insurance cashless garages list in '.$city_name;
            }

            if($garage_count && $insurer_display_name && $city_name){
                $title = number_format($garage_count) . '+ ' .$insurer_display_name.' Cashless Garages list in '.ucwords($city_name).' | Turtlemint';

                $description = 'Check the wide network of '.number_format($garage_count).'+ '.$insurer_display_name.' cashless garages in '.ucwords($city_name).'. Search for nearby authorised cashless garages for your vehicle.';
            }else{
				$title = $insurer_display_name.' Cashless Garages list in '.ucwords($city_name).' | Turtlemint';

                $description = 'Check the wide network of '.$insurer_display_name.' cashless garages in '.ucwords($city_name).'. Search for nearby authorised cashless garages for your vehicle.';
			}
            if($title){
        ...
        <script type="application/ld+json">
        {
            ...
            "@type": "WebPage",
            "name": "<?php echo $insurer_display_name; ?> Cashless Garages List in <?php echo ucwords($city_name); ?>",
            "speakable":
            {
    ...
    <body <?php body_class('tm-style-1'); ?>>
        ...
        <div class="wp-site-blocks">
            ...
            <?php echo do_shortcode('[tm_cg_banner type="cg-list"]'); ?>
            ...
            <section class="wp-block-group has-global-padding is-layout-constrained">
                <div class="wp-block-columns alignwide is-layout-flex ic-two-column">
                    <div class="wp-block-column is-layout-flow left-scrollable" style="flex-basis:65%">
                        <?php echo do_shortcode('[tm_cg_list city="'.$city_name.'" insurercode="'.$insurer_code.'" ]'); ?>
                        <?php echo do_shortcode('[tm_ic_calculator vertical="car"]'); ?>
                        <?php echo do_shortcode('[tm_cg_nearby_insurers city="'.$city_name.'" insurercode="'.$insurer_code.'"]'); ?>
                        ...
                        <?php echo do_shortcode('[tm_cg_popular_city type="car" page="CG-City"]'); ?>
                    </div>
                    <div class="wp-block-column is-layout-flow sticky-column" style="flex-basis:35%">
                        <?php echo do_shortcode('[tm_cg_nearby_cities city="'.$city_name.'" insurercode="'.$insurer_code.'" ]'); ?>
