<?php
function tmIcNhStates($atts){

	$atts = shortcode_atts( 
    array(
      'type' => ''
    ), $atts, 'tm_ic_nh_states' );
  ...
  <?php
        ...
        $stateData = getCitiesByState($insurer_code);
              ...
              <div class="alignwide">
                <div class="wp-block-columns is-stacked-on-mobile mb-0">
                  <div class="wp-block-column" style="flex-basis: 65%;">
                    <h2 class="section-heading has-turtlemint-child-tm-x-large-font-size"><?php echo $insurer_display_name.ucwords(" Cashless Hospitals List in India"); ?></h2>
                  </div>
                  <div class="wp-block-column has-text-align-md-right" style="flex-basis: 35%;">
                    <div class="count-wrapper">Total Hospitals: <?php echo $hospital_count;?></div>
                  </div>
                </div>
                <div class="tm-container__content mb-0">
                    <p>There are more than <?php echo $hospital_count;?> <?php echo ucwords($insurer_display_name); ?> health insurance network hospitals in India, which allows flexibility of choice while opting for the best hospitals. Hence, the network of cashless hospitals is one of the important deciding factors when choosing a health insurance plan. <?php echo ucwords($insurer_display_name); ?> Health Insurance Company processes claims in a timely and efficient manner to reduce the turnaround time and to benefit the policyholders better. Find the details of <?php echo ucwords($insurer_display_name); ?> Health Insurance network hospitals in India below</p>
                                            ...
                                            <div class="tm-accordion-body">
                                                ...
                                                <?php
                                                    ...
                                                    foreach ($cities as $city) {?>
                                                        <a onclick="tmClickEvent('State city clicked', 'NH-India', '<?php echo $insurer_display_name; ?>', '<?php echo ucwords($city);?>')" title="<?php echo $insurer_display_name; ?> Health Insurance Hospital List in <?php echo ucwords($city);?>" href="<?php echo get_site_url().'/health-insurance/'.$tax_slug.'/network-hospitals/'.str_replace(' ', '-', $city).'/'?>"><?php echo ucwords($city);?></a>
                                                    <?php }
                                                ?>
                                                </div>
                                            </div>
...
add_shortcode('tm_ic_nh_states', 'tmIcNhStates');
?>