<?php
function tmCgSearchBox($atts){
    
	$atts = shortcode_atts( array(
        'city' => '',
        'insurercode' => ''
    ), $atts, 'tm_cg_search_box' );
        ...
        <section class="highlight-section tm-container blurred-bg bordered tm-container" id="nh-search-box">
            <div class="wp-block-group has-global-padding is-layout-constrained">
                <div class="wp-block-column" style="flex-basis: 75%;">
                    <h2 class="section-heading has-turtlemint-child-tm-x-large-font-size"><?php echo $insurer_display_name;?><?php echo ucwords(" Cashless Garages "); ?></h2>
                    <div class="tm-container__content">
                        <p><?php echo $insurer_display_name;?> has a network of <?php echo $garage_count;?> cashless garages across India. To find the nearest <?php echo $insurer_display_name;?> authorised cashless garage, enter your location below:</p>
                    </div>
                    <form class="form-style-1 nh-banner-searchform d-block">
                        <div class="field-group field-group-search">
                            ...
                            <?php echo do_shortcode('[tm_cg_popular_city page="CG-India" uitype="list"]'); ?>
                        ...
                        <div class="wp-block-columns popular-cities-list mb-0">
                            <?php
                                foreach ($popular_cities_section as $popular_city):
                                    ...
                                    global $wpdb;
                                    $city_hospotal_count = $wpdb->get_var( "SELECT count FROM `wp_mdcsm8_cashless_garages_insurers` as i INNER JOIN `wp_mdcsm8_cashless_garages_state_city` as c ON i.cityid = c.id WHERE c.city = '{$city_name}' AND i.insurer = '{$insurer_code}'" );
                                ?>
                                <?php 
                                if($city_hospotal_count > 0 ) { 
                                    if ($popular_city_title && $popular_city_logo):?>
                                    <a href="<?php echo get_site_url().'/car-insurance/'.$tax_slug.'/cashless-garages/'.strtolower(str_replace(' ', '-', $popular_city_title)).'/'?>" class="popular-city">
...
add_shortcode('tm_cg_search_box', 'tmCgSearchBox');
?>