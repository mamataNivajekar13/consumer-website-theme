<?php
function tmIcNhPopularCity($atts){

	$atts = shortcode_atts( array(
    'type' => '',
    'page' => '',
    'uitype' => '',
  ), $atts, 'tm_ic_nh_popular_city' );
  ...
  <?php
    ...
    $current_page_city = (get_query_var( 'health-ic-city' ));
        ...
        <?php if($atts['uitype'] != "list") {?>
          <section id="popular-cities" class="highlight-section tm-container blurred-bg bordered">
              <h2 class="section-heading has-turtlemint-child-tm-x-large-font-size">
                  <?php echo ucwords($insurer_display_name . " Health Insurance Network Hospitals in Popular Cities"); ?>
              </h2>
              <div class="wp-block-columns popular-cities-list mb-0">
                      ...
                      <?php 
                        if($city_hospotal_count > 0 ) { 
                          if ($popular_city_title && $popular_city_logo):?>
                            <a onclick="tmClickEvent('Popular city clicked', '<?php echo $atts['page'];?>', '<?php echo $insurer_display_name; ?>', '<?php echo $popular_city_title;?>')" href="<?php echo get_site_url().'/health-insurance/'.$tax_slug.'/network-hospitals/'.strtolower(str_replace(' ', '-', $popular_city_title)).'/'?>" class="popular-city">
                                <div class="popular-city-icon has-turtlemint-child-tm-light-purple-background-color">
                                  <img loading="lazy" title="<?php echo ucwords($insurer_display_name . " Network Hospitals In ".$popular_city_title); ?>" alt="<?php echo ucwords($insurer_display_name . " Health Insurance Hospital List in ".$popular_city_title); ?>" src=<?php echo $popular_city_logo;?> />
                                </div>
                ...
                <?php if($templateName !='ic-network-hospitals.php' ) { ?>
...
add_shortcode('tm_ic_nh_popular_city', 'tmIcNhPopularCity');
?>