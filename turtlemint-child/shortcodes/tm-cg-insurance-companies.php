<?php
function tmCgInsuranceCompanies($atts){

	$atts = shortcode_atts(array(
    'limit' => 5,
    'classes' => ''
  ), $atts, 'tm_cg_insurance_companies' );
      ...
      <div class="wp-block-columns is-not-stacked-on-mobile mb-0">
        <div class="wp-block-column" style="flex-basis: 75%;">
          <?php if(isset($tax_slug) && $tax_slug != ''):?>
            <h2 class="section-heading has-turtlemint-child-tm-x-large-font-size"><?php echo ucwords(" Other Car Insurance Cashless Garages In India "); ?></h2>
          <?php else:?>
            <h2 class="section-heading has-turtlemint-child-tm-x-large-font-size"><?php echo ucwords("Check the Cashless Garages across insurers"); ?></h2>
          <?php endif;?>
          <div class="tm-container__content">
            <?php if(isset($tax_slug) && $tax_slug != ''):?>
              <p>Check the list of cashless garages of other car insurance companies below:</p>
            <?php else:?>
              <p>Check the list of car cashless garages across motor insurance companies below:</p>
            <?php endif;?>
          </div>
        </div>
      ...
      <div class="viewMoreContainer has-turtlemint-child-white-background-color" style="margin-bottom:16px">
        <?php foreach($insurer_features_set1 as $child_term_id): ?>
          ...
          <div class="tm-insurance-company">
              <a class="tm-insurance-company__title" href="<?php echo get_term_link($child_term_id); ?>cashless-garages/" title="<?php echo ucwords($ic_display_name." Cashless Garages List"); ?>" onclick="tmClickEvent('Other network garage', 'CG-India', '<?php if(isset($event_label)): echo $event_label; endif; ?>', '<?php echo $ic_display_name;?>')">
                <?php echo ucwords($ic_display_name." Cashless Garages List"); ?>
...
add_shortcode('tm_cg_insurance_companies', 'tmCgInsuranceCompanies');
?>