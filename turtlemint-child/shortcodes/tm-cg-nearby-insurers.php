<?php
function tmCgNearbyInsurers($atts){

	$atts = shortcode_atts( array(
    'city' => '',
    'insurercode' => '',
    'limit' => 5
  ), $atts, 'tm_cg_nearby_insurers' );
  ...
  <?php
    ...
    global $wpdb;
    $nearby_insurers = $wpdb->get_col( "SELECT DISTINCT insurer FROM {$wpdb->prefix}cashless_garages_insurers as i RIGHT JOIN {$wpdb->prefix}cashless_garages_state_city as c on i.cityid = c.id WHERE insurer != '{$insurer_code}' AND i.count > 0 AND c.city = '{$cityname}' ORDER BY i.count;" );
          ...
          <div class="tm-container has-turtlemint-child-tm-light-purple-background-color">
            <div class="wp-block-columns is-not-stacked-on-mobile mb-0">
              <div class="wp-block-column" style="flex-basis: 75%;">
                <h2 class="section-heading has-turtlemint-child-tm-x-large-font-size"><?php echo ucwords("Other Cashless Garages In "); ?><?php echo ucwords($atts['city'])?></h2>
                <div class="tm-container__content">
                  <p>One of the key aspects considered while buying a car/vehicle insurance plan is the list of authorised cashless garages network of the insurer, as it ensures smooth claim process with less turnaround time. Find the list of cashless garages of different insurers in <?php echo ucwords($atts['city'])?> below:</p>
                </div>
              </div>
            ...
            <div class="viewMoreContainer has-turtlemint-child-white-background-color" style="margin-bottom:16px">
              <?php foreach($nearby_insurers as $nearby_insurer): ?>
                ...
                <div class="tm-insurance-company">
                    <a class="tm-insurance-company__title" href="<?php echo get_site_url().'/car-insurance/'.$term->slug.'/cashless-garages/'.str_replace(' ', '-', strtolower($atts['city'])).'/'?>" title="<?php echo $ic_display_name. " Car Insurance Garage List In " . ucwords($atts['city'])?>" onclick="tmClickEvent('Other Cashless Garages', 'CG-City', '<?php echo $insurer_display_name; ?>', '<?php echo ucwords($ic_display_name)?>', '<?php echo ucwords($atts['city']);?>')">
                      <?php echo $ic_display_name. " Cashless Garage List In " . ucwords($atts['city'])?>
...
add_shortcode('tm_cg_nearby_insurers', 'tmCgNearbyInsurers');
?>