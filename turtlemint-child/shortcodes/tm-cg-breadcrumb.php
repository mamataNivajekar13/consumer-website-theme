<?php
function tmCgBreadcrumb($atts){

	$atts = shortcode_atts( array(
    'type' => ''
  ), $atts, 'tm_cg_breadcrumb' );
    ...
    if($type == 'custom' && $breadcrumb_links){
      if(get_field('insurer_name', $term)){
        ...
        $insurer_display_name = ucwords($term->name);
          ...
          ?>
          <div class="tm-breadcrumb mt-0 mb-0" id="breadcrumbs">
            ...
            <span title="<?php echo ucwords($insurer_display_name." ".$vertical_name." Insurance Cashless Garages"); ?>"><?php echo ucwords("Cashless Garages"); ?></span>
          </div>
         <?php 
    } elseif($type == 'cg-list' && $breadcrumb_links ) {
      if(get_field('insurer_name', $term)){
        $insurer_display_name = ucwords(get_field('insurer_name', $term));
      }else{
        $insurer_display_name = ucwords($term->name);
        ...
        <div class="tm-breadcrumb mt-0 mb-0" id="breadcrumbs">
          ...
          <span>
            <a href="<?php echo home_url().'/'.$breadcrumb_links[0]['link']."/".$tax_slug."/cashless-garages"; ?>" title="<?php echo ucwords($insurer_display_name." ".$vertical_name." Insurance Cashless garages"); ?>"><?php echo ucwords("Cashless Garages"); ?>
            </a>
          </span>&gt;
          <span title="<?php echo ucwords($insurer_display_name." ".$vertical_name." Insurance Cashless garages In ".$city_name); ?>"><?php echo ucwords($city_name); ?></span>
        </div>
      <?php
    } else{
...
add_shortcode('tm_cg_breadcrumb', 'tmCgBreadcrumb');
?>